...
export default function DashboardNavBar({ username, image, id }: navBarProps) {
  ...
  return (
    <div className="absolute z-50 flex flex-col items-center w-full">
      <div className="flex flex-row items-center md:justify-between p-2 md:p-5 bg-background_secondary border-b border-border w-full gap-5 md:gap-10">
        <Link href="/">
          <AppLogo width={30} height={30} text_color="base" />
        </Link>
        <div id="search" className="w-2/3 md:w-1/3 items-center flex flex-row border rounded-2xl text-text border-text_muted hover:border-border p-2">
      ...
      {isOpen && (
        <div className="overflow-y-auto overflow-x-hidden h-1/2 flex flex-col mt-0.5 bg-background_secondary border border-border text-text shadow-2xl w-2/3 md:w-1/2 transition-all rounded-2xl p-10 space-y-3">
