...
export default function DashboardMain({ items }: { items: any }) {
  ...
  return (
    <div className="h-full flex flex-col p-10 mt-10 md:mt-0 md:p-30 gap-15 overflow-x-hidden max-w-full">
      {optimisticDocs.length <= 0 ?

        <div className="flex flex-col items-center justify-center gap-5">
          <h1 className="text-text text-2xl md:text-4xl">There are not documents, let's start by creating one here</h1>
          <button onClick={createDoc} className="bg-primary hover:bg-secondary p-2 text-xs md:text-xl rounded-2xl cursor-pointer transition-all">New Document</button>
        </div> :
        <div className="gap-3">
          <button onClick={createDoc} className="bg-primary hover:bg-secondary p-2 rounded-2xl cursor-pointer transition-all mb-5">New Document</button>
          <h1 className="text-3xl text-text font-semibold">Your Documents</h1>
          <div className="grid grid-cols-1 md:grid-cols-5 p-5 md:p-10 gap-5">{optimisticDocs.map((doc: any, i: number) => (
      ...
      {items.collabDocs.length <= 0 ?
        <div className="w-full flex items-center justify-center align-items">
          <h1 className="text-text text-2xl md:text-4xl">You are a not collaborator of any document, ask a friend for a link!</h1>
        </div> :
        <div>
          ...
          <div className="flex flex-row overflow-x-auto overflow-y-hidden gap-10 p-10 w-full">
            {items.collabDocs.map((doc: any, i: any) => (
              <Link key={i} href={`/document/${doc.id}`} className="bg-primary p-5 md:p-10 rounded-2xl">
                <div className="flex items-center justify-center align-items text-zinc-800 font-semibold text-2xl">
                  {doc.title}
                </div>
